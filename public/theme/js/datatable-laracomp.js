...
$.fn.dtTableLaraComp = function (param = {}) {
    ...
    attr["default_filter"] = param.hasOwnProperty("default_filter")
        ? param.default_filter
        : {};
    attr["checkbox"] = param.hasOwnProperty("checkbox")
        ? param.checkbox
        : false;
    attr["checkbox_action"] = param.hasOwnProperty("checkbox_action")
        ? param.checkbox_action
        : false;
...
var CustomToolsLaravelDatatable = function (tableId) {
    ...
    var handleFilter = () => {
        $(document).on(
            "click",
            `.act-filter_reset[data-table="${table}"], .act-filter_applay[data-table="${table}"]`,
            function () {
                if ($(this).hasClass("act-filter_reset"))
                    resetForm(`${table}-filter_form`);
    ...
    var customFilter = function () {
        ...
        if (document.getElementById(`${table}-filter_form`)) {
            ...
            $(`.${table}-trigger_filter #filter-count`).html(
                `${countFilter > 0 ? "(" + countFilter + ")" : ""}`,
            );
        }
        // add daefault filter
        if (defaultFilter) {
    ...
    var handleOrder = function () {
        ...
        $(document).on(
            "click",
            `#${table}-order_form .add-custom_order`,
            function () {
                var order_kolom = $(
                    `#${table}-order_form [name="order_kolom"]`,
                ).val();
                var order_type = $(
                    `#${table}-order_form [name="order_type"]`,
                ).val();

                if (order_kolom && order_type) {
                    var text_order_kolom = $(
                        `#${table}-order_form [name="order_kolom"]`,
                    )
        ...
        $(document).on(
            "click",
            `.act-order_applay[data-table="${table}"], .act-order_reset[data-table="${table}"]`,
            function () {
                ...
                var formElements = document.getElementsByClassName(
                    `${table}-custom-order`,
                );
                var dataOrder = [];
                for (var i = 0; i < formElements.length; i++) {
                    var element = formElements[i];
                    if (element.name && element.value && element.value != "0")
                        dataOrder.push([
                ...
                $(`.${table}-trigger_order #order-count`).html(
                    `${dataOrder.length > 0 ? "(" + dataOrder.length + ")" : ""}`,
                );
    ...
    var checkboxAction = function () {
        $(document).on(
            "change",
            `#${table} .checkable, .${table}-check_all`,
            function () {
                var checkboxAction = $(`#${table}-checkbox_action`);
                var selected = $(`#${table} .checkable:checked`).length;
                $(`#${table}-checkbox_action #checked_row-count`).html(
                    selected + " data",
                );
    ...
    var exportXlsExternal = function (param) {
        ...
        function loadData(start) {
            ...
            if (currLoop <= maxLoop) {
                ...
                if (alldata.length > 0) {
                    var objectKeys = Object.keys(alldata[0]);
                    var sampleData = alldata.slice(0, 1000);

                    var colWidths = objectKeys.map((key) => {
                        var maxLen = sampleData.reduce((max, row) => {
                            var val = row[key] ? row[key].toString() : "";
                            if (val.includes("\n")) {
                                var subLines = val.split("\n");
                                var subMax = Math.max(
                                    ...subLines.map((l) => l.length),
                                );
