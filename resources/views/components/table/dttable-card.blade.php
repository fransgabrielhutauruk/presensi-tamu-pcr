...
@php
    ...
    if ($builder) {
        ...
        $domCard = "
                    ...
                    <'row'<'col-sm-12 d-flex align-items-$align justify-content-$align mt-8'p>>
                ";
        ...
        $builder
            ...
            ->drawCallback('function(settings) {' . ($table_card ? 'tableCard' . $builder->getTableId() . '();' : '') . '' . $drawCallback . '}')
...
<div class="{{ $covered ? 'border border-gray-200 rounded-2 p-3' : '' }}">
    ...
    <div class="mt-3 {{ $responsive !== true ? 'table-responsive-' : '' }}">
        ...
        @endif
        <div id="tableCard{{ $builder->getTableId() }}" class="row table-card justify-content-{{ $align }}" {{ $attributes }}></div>
