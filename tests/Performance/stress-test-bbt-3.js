...
export const options = {
    scenarios: {
        stress_test_presensi: {
            executor: "ramping-arrival-rate",
            startRate: 50, // Mulai langsung dengan 50 pengguna/detik
            timeUnit: "1s", // Hitungan per 1 detik
            preAllocatedVUs: 500, // Siapkan 500 VU di memori lokal sebagai pekerja
            maxVUs: 2000, // Izinkan k6 menambah hingga 2000 pekerja jika server melambat
            stages: [
                { duration: "1m", target: 50 }, // Tahan di 50 request/detik selama 1 menit
                { duration: "1m", target: 100 }, // Naik ke 100 request/detik
                { duration: "1m", target: 200 }, // Naik ke 200 request/detik
                { duration: "1m", target: 500 }, // Lonjakan ekstrem ke 500 request/detik
                { duration: "1m", target: 1000 }, // Hantaman ribuan request/detik (Titik hancur)
            ],
        },
    },
    // === TAMBAHKAN BLOK THRESHOLDS INI ===
    thresholds: {
        // Aturan: Tingkat kegagalan HTTP (http_req_failed) harus 0 (rate==0).
        // abortOnFail: true -> Langsung matikan k6 jika aturan di atas dilanggar.
...
export function setup() {
    ...
    if (!eventId) {
        throw new Error(
            "event_id valid tidak ditemukan. Set env EVENT_ID dari database/URL event yang aktif.",
        );
    }
...
export default function (setupData) {
    const eventId =
        setupData && setupData.eventId ? setupData.eventId : EVENT_ID_FROM_ENV;
    check(
        { eventId },
        {
    ...
    const payload = {
        ...
        instansi: `Instansi Stress ${suffix.slice(-6)}`,
    ...
    if (submitResponse.status === 419) {
        ...
        const refreshBody =
            typeof refreshResponse.body === "string"
...
export function handleSummary(data) {
    return {
        [`laporan-performa-stress-test-bbt-3-${Date.now()}.html`]:
            htmlReport(data),
