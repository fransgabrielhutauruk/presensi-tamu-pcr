...
class KontenPage extends Model
{
    use SoftDeletes;
    use LogsActivity;
    /**
     ...
     * @var string
     */
    protected $primaryKey = 'kontenpage_id';
    ...
    {
        $query = DB::table('')
            ->selectRaw('a.*, t.nama_tipe, t.kode_tipe')
            ->from((new self)->table . ' as a')
            ->join('konten_tipe as t', function (JoinClause $join) {
                $join->on('t.kontentipe_id', '=', 'a.kontentipe_id');
            })
            ->where(notRaw($where))
